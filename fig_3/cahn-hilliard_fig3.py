...
def save_snapshot_figures(snapshots, snapshot_steps, save_path):
    ...
    colorbar PDF and PNG (axis/ticks kept, no tick labels), all alongside the MP4
    (same directory/basename as save_path).
    """
...
if __name__ == '__main__':
    ...
    mp4_freq = 10 #Output every nth frame to mp4 file

    order_params = [0.2] # Take (order_param + 1)/2 to get solvent ratio. Fig 3: order_param = 0.2. Fig S22: order_param = 0.6

    # Time step indices (t) to extract as individual snapshot figures (PDF/PNG)

    snapshot_frames = (0, 3990, 28980)
    ...
    for gamma in gammas:
        for order_param in order_params:
            ...
            save_path = str(SCRIPT_DIR / f'fig3_{round(1/2*(order_param+1),2)}.mp4')
